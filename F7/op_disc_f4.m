...
for oi = 1:length(ontim)
    ...
    cmn = slidingmean([],opm.mean,cm,-1);
    ce1 = slidingmean([],opm.ci(1,:),cm,-1);
    ce2 = slidingmean([],opm.ci(2,:),cm,-1);
    ce = [ce1;ce2];
    
    plot_bci([],ce,cmn,cclr,[],ls)
    fln = length(opm.mean);
    plotstandard
    set(gca,'xlim',[0 fln],'xtick',[0 (opr*3):180:fln],...
        'ylim',[0.01 .065],'ytick',0.02:0.02:.06,'yticklabel','')
end